...
- (void)getAllPhoto:(void (^)(NSArray *))result
{
    NSMutableArray *mArray = [[NSMutableArray alloc] init];
    [self getGroupList:^(NSArray *array) {
        for (int i = 0; i<array.count; i++) {
            ALAssetsGroup *group = array[i];
            [self getPhotoListOfGroup:group result:^(NSArray *arrays) {
                [mArray addObjectsFromArray:arrays];
            }];
        }
        result(mArray);
        return ;
